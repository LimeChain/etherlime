...
const run = async (transactionHash) => {
	...
	// let contracts = await fileRead();
	// let bugger = await Debugger
	// 	.forTx(transactionHash, { contracts, files, provider });
	...
	var sessionPromise = compilePromise
		.then(function (result) {
			...
			return Debugger.forTx(transactionHash, {
				provider: provider,
				files: files,
...
const fileRead = async () => {
	...
	fs.readdirSync(testFolder).forEach(function (file) {
		contracts[file.replace(/\.json$/, '')] = require(testFolder + '/' + file);
	});
	return contracts;
}
...
// 	const argv = yargs
// 		.command('add', 'Add a new note with this command', {
// 			test,
// 		})
// 		.help()
// 		.argv;
