...
describe('etherlime verify command', () => {
	...
	it('should execute etherlime verify with optional params', async () => {
		const expectedOutput = '===== Verifying Completed. Please check output.json ====='
		const childResponse = await runCmdHandler(`etherlime verify --public_signals=circuit_public_signals.json --proof=circuit_proof --verifier_key=circuit_verification_key.json`, expectedOutput);
		assert.include(childResponse.output, expectedOutput, 'The verify process does not finish properly');
	});
	...
	it('should throw on verify error', async () => {
		...
		const childResponse = await runCmdHandler(`etherlime verify --verifier_key=circuit_verification_key-with-error.json`, expectedOutput);
